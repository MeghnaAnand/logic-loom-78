...
const level4: Record<BlockType, Record<CodeLanguage, CodeGen>> = {
  ...
  data: {
    ...
    n8n:        (b) => `{ "name": "${b.label}", "type": "n8n-nodes-base.set",\n  "parameters": { "values": "={{$json.map(transform).filter(validate)}}" } }`,
    pseudocode: (b) => `FETCH ${b.label.toLowerCase()}\nFOR EACH record\n  TRANSFORM record\n  IF valid → KEEP\nEND FOR`,
  },
  output: {
...
const level5: Record<BlockType, Record<CodeLanguage, CodeGen>> = {
  trigger: {
    ...
    n8n:        (b) => `{ "name": "${b.label}", "type": "n8n-nodes-base.webhook",\n  "parameters": { "path": "/${toFuncName(b.label, "snake")}",\n    "method": "POST", "authentication": "headerAuth" } }`,
  ...
  condition: {
    ...
    n8n:        (b) => `{ "name": "IF", "type": "n8n-nodes-base.if",\n  "parameters": { "conditions": {\n    "number": [{ "value1": "={{$json.score}}",\n      "operation": "greaterThan", "value2": 80 }] } } }`,
  ...
  action: {
    ...
    n8n:        (b) => `{ "name": "${b.label}", "type": "n8n-nodes-base.httpRequest",\n  "parameters": { "method": "POST", "url": "={{$env.API_URL}}",\n    "retryOnFail": true, "maxRetries": 3,\n    "authentication": "oAuth2" } }`,
  ...
  output: {
    ...
    n8n:        (b) => `{ "name": "${b.label}", "type": "n8n-nodes-base.httpRequest",\n  "parameters": { "method": "POST",\n    "url": "={{$env.OUTPUT_ENDPOINT}}",\n    "body": "={{$json}}",\n    "options": { "timeout": 15000, "redirect": "follow" } } }`,
...
export function getFullCode(blocks: Block[], language: CodeLanguage, level: number = 1): string {
  ...
  const header: Record<CodeLanguage, string> = {
    ...
    n8n: `// n8n Workflow JSON — ${levelLabel}\n`,
